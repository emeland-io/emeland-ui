...
<template>
  ...
  >
    <div class="border-b border-border-1 px-6 py-4">
      <div class="flex items-start justify-between gap-4">
        <h2 class="text-base font-medium text-text-1">{{ system.displayName }}</h2>
        <div class="flex items-center gap-1.5">
          <span class="font-mono text-xs text-text-4">{{ system.systemId }}</span>
          <CopyButton
      ...
      <div v-if="contexts.length > 0">
        ...
        <div class="flex flex-wrap gap-1.5">
          <button
            ...
            :key="ctx.id"
            class="rounded bg-accent/10 px-2 py-0.5 font-mono text-xs text-accent transition-opacity hover:opacity-80"
      ...
      <div v-if="system.parent">
        ...
        >
          ...
          <div class="ml-auto flex shrink-0 items-center gap-1.5">
            <span class="font-mono text-[11px] text-text-4">{{ system.parent }}</span>
            <CopyButton
              :value="system.parent"
              :size="12"
              @click.stop
      ...
      <div v-if="instances.length > 0">
        ...
        >
          <div class="flex items-center gap-2.5">
            <span
              class="truncate text-sm text-text-2 transition-colors group-hover/row:text-accent"
            >
              {{ inst.displayName }}
            </span>
            <span
            ...
            <span class="ml-auto flex shrink-0 items-center gap-1.5">
              <span class="font-mono text-[11px] text-text-4">{{ inst.systemInstanceId }}</span>
              <CopyButton
  ...
  >
    <span class="font-mono text-xs text-text-4">Select a system to inspect</span>
  </div>
</template>
